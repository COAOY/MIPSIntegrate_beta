...
#
#                                  pt2tmax
#                  Copyright (c) 1998-2015 by Synopsys, Inc.
#                             ALL RIGHTS RESERVED
#
#
...
proc write_delay_paths {num args} {
  ...
  foreach n $nlist {
	...
	if {$n == "-max_paths"} {
		...
		echo "       \[-net <pin_name>\] \[-cell <pin_name>\]"
		echo "       \[-delay_type <max|min>\] \[-pba\] <file_name>\n"
		...
		echo "		-pba			: Selects exhaustive path based analysis for path selection"
...
proc write_exceptions_from_violations {args} {
	...
	set position [lsearch -exact $args {-full_update_timing}]
	if {$position != -1} {
		set full_update_timing 1
		set newargs [lreplace $args $position $position]
		set args $newargs
	}
	set position [lsearch -exact $args {-pba}]
	if {$position != -1} {
		set pba_mode path
		set newargs [lreplace $args $position $position]
		set args $newargs
	...
	foreach {name value} $args {
		if {$name == "-output"} {
			...
			echo "Usage: write_exceptions_from_violations \[-output <filename>\] \[-specific_start_pin\] \[-full_update_timing\] \[-max_iterations <number>\] \[-delay_type <max|min|min_max>\]"
			...
			echo "    -full_update_timing"
			echo "        Force full timing update for 2nd and later iterations."
			echo "        Use if too many violating paths causes excessive update_timing runtime."
			echo "    -pba"
			echo "        Use path-based analysis to reduce pessimism at the expense of runtime."
	...
	for {set iter_num 1} {$iter_num <= $max_iter_num} {incr iter_num} {
		...
		set paths [get_timing_paths -start_end_pair -delay_type $delay_type -slack_lesser_than 0 -pba_mode $pba_mode]
